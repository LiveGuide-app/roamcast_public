...
const getEnvironment = (): 'development' | 'test' | 'production' => {
  // Check for Expo's environment variables
  if (process.env.EXPO_PUBLIC_ENV) {
    return process.env.EXPO_PUBLIC_ENV as 'development' | 'test' | 'production';
  }
  ...
  // Check for __DEV__ which is available in React Native/Expo
